...
export async function POST(req: NextRequest) {
  ...
  if (!name || !tasks || !Array.isArray(tasks)) {
    return NextResponse.json({ error: 'Missing name or tasks array' }, { status: 400 })
  }

  const { data, error } = await (supabase as any)
    .from('checklist_templates')
    .insert({ user_id: session.user.id, name, description: description || null, tasks })
    .select()
    .single()
...
export async function PUT(req: NextRequest) {
  ...
  const { data, error } = await (supabase as any)
    .from('checklist_templates')
    .update({ name, description, tasks, updated_at: new Date().toISOString() })
    .eq('id', id)
    .eq('user_id', session.user.id)
    .select()
    .single()
