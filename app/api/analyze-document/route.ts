  ...
  try {
    ...
    if (isPDF) {
      messageContent = [
        ...
        { type: 'text', text: `You are an expert real estate transaction coordinator. Analyze this document and respond ONLY with valid JSON (no markdown) with this structure: {"summary":"2-3 sentence summary","parties":{"buyer":null,"seller":null,"buyerAgent":null,"sellerAgent":null,"titleCompany":null,"lender":null},"keyTerms":{"purchasePrice":null,"closingDate":null,"earnestMoney":null,"downPayment":null,"loanAmount":null,"propertyAddress":null,"propertyType":null},"contingencies":[{"name":"string","deadline":null,"status":"pending"}],"criticalDates":[{"label":"string","date":"string","daysUntil":0}],"actionItems":[{"priority":"high","task":"string","responsible":"agent"}],"risks":[{"severity":"high","issue":"string"}],"documentType":"Purchase Agreement","completionScore":85} Today is ${new Date().toISOString().split('T')[0]}.` }
      ]
    } else {
      // For non-PDF files, send as text prompt with filename context
      messageContent = [
        { type: 'text', text: `You are an expert real estate transaction coordinator. I have a real estate document called "${fileName}". Based on the filename and typical real estate document contents, provide a realistic analysis. Respond ONLY with valid JSON (no markdown) with this structure: {"summary":"2-3 sentence summary about what this document type typically contains and its purpose","parties":{"buyer":null,"seller":null,"buyerAgent":null,"sellerAgent":null,"titleCompany":null,"lender":null},"keyTerms":{"purchasePrice":null,"closingDate":null,"earnestMoney":null,"downPayment":null,"loanAmount":null,"propertyAddress":null,"propertyType":null},"contingencies":[{"name":"string","deadline":null,"status":"pending"}],"criticalDates":[{"label":"string","date":"string","daysUntil":0}],"actionItems":[{"priority":"high","task":"string","responsible":"agent"}],"risks":[{"severity":"medium","issue":"string"}],"documentType":"Closing Disclosure","completionScore":75} Today is ${new Date().toISOString().split('T')[0]}. Note: This file is a .docx format which requires conversion to PDF for full content extraction. The analysis below is based on document type recognition.` }
      ]
    }
