  ...
  try {
    ...
    if (isPDF) {
      ...
      messageContent = [
        ...
        { type: 'text', text: `This is a photo/scan of a real estate document called "${fileName}". Read ALL text visible in the image carefully. ${analysisPrompt}` }
      ]
    } else {
      // For other file types (doc, docx, etc.), send as text with filename context
      let textContent = ''
      try { textContent = Buffer.from(docBuffer).toString('utf-8') } catch { textContent = '' }
      messageContent = [
        { type: 'text', text: `This is a real estate document called "${fileName}". Here is the document content:\n\n${textContent.slice(0, 50000)}\n\n${analysisPrompt}` }
      ]
    }
